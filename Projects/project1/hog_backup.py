...
def run_experiments():
    ...
    if True: #test winning strat
        result = eval_strategy_range(a_strategy,3,20)
        print('Best winning strat num:', result)
...
def a_strategy(x, num_rolls = 5):
    min_points = x
    def function(score, opponent_score): #score , opponent score
        min_points = 4
        #free_bacon = opponent_score // 10 + 1
        n = 5
        ...
        if (score - opponent_score) >= 13:
            n = 4
        if (opponent_score - score) >= x: #lose margin
            n = 6
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    """def make_final_strategy(x, num_rolls = 5):
        def function
    ...
    def compare_strategies(strategy, baseline=always_roll(5)):
        as_first = 1 - make_average(play, 10)(strategy, baseline)
        as_second = make_average(play, 10)(baseline, strategy)
        return (as_first + as_second) / 2  # Average the two results


    result = eval_strategy_range(make_final_stategy(score,opponent_score), 1, 25)
   """ 
    ...
    if (score - opponent_score) >= 13:
        n = 4
    if (opponent_score - score) >= 7: #lose margin
        n = 6
    if (opponent_score - score) >= 20: #lose margin
        n = 7
    if (score - opponent_score) >= 20:
    ...
    """if opponent_score > 90:
        n = 0
    if score > 91 and opponent_score > 80:
        n = 0
    if score > 92 and opponent_score > 70:
        n = 0
    if score > 86 and opponent_score > 80:
        n = 0"""
    """if (min_points < free_bacon) and (((score + free_bacon + opponent_score)%10 == 7) or ((score + free_bacon + opponent_score)%7 == 0)):
        n = 0"""
    if (opponent_score - score) >= 5:
        return make_comeback_strategy(5, n)(score, opponent_score)
    if ((score + opponent_score)%10 == 7) and opponent_score > 30:
        return make_mean_strategy(min_points, n)(score, opponent_score)
    return n
