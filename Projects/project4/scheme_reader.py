...
def scheme_read(src):
    ...
    if val == "nil":
        ...
        #essentially makes a list of quote and the expression that was afte the quote
...
def read_tail(src):
    ...
    if src.current() == ")":
        ...
        return nil
    "*** YOUR CODE HERE ***"
    #checks if the first is a period
    if src.current() == '.':
        src.pop()
        #reads the rest of the list
        first = scheme_read(src)
        rest = read_tail(src)
        #if the rest is nil, the expression is not correct
        if rest != nil:
            raise SyntaxError('Expected one element after .')
        return first
    first = scheme_read(src)
    rest = read_tail(src)
    return Pair(first, rest) #lastly makes a pair of the first and rest (dotted)
