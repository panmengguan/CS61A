...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        QueenAnt.number_instance += 1
        self.instance = QueenAnt.number_instance
        self.buffed_ants = []
        ...
        if self.instance != 1:
            ...
            while location.name != 'AntQueen':
                if location.ant != None:
                    if type(location.ant) is BodyguardAnt: #
                        if location.ant.ant != None:
                            if location.ant.ant not in self.buffed_ants:
                                location.ant.ant.damage = 2*location.ant.damage
                                self.buffed_ants += [location.ant.ant,]
                    else:
                        if location.ant not in self.buffed_ants:
